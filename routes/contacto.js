export function renderContacto(container) {
  container.innerHTML = `
    <section class="contact">
      ...
      <div class="map-container mt-3" style="border-radius:8px; overflow:hidden; max-width: 600px; height: 250px;">
        <iframe
  ...
  function getMensajeDesdeURL() {
    const params = new URLSearchParams(window.location.search);
    return params.get('mensaje') || '';
  }

  const mensajePreCargado = getMensajeDesdeURL();
  const textarea = container.querySelector('#mensaje');
  textarea.value = mensajePreCargado;
